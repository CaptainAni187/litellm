...
def member_may_write_auto_router(
    ...
    through ``model_name``; an existing row must be one the member created; and a public name the
    write registers must not widen the team's model access.
    """
    ...
    if effective_kind is None or (write.stored is not None and stored_kind is None) or write.reads_server_file():
        return False
    if write.moves_team() or write.renames_outside_the_rename_channel():
        return False
